function moleculecalculateMoleculeWeight(molecularFormula)
    % Load element information
    ElementsInput(); 
    global Elements; 
    
    % Create a map for efficient element-to-atomic-weight lookup
    atomicWeights = containers.Map(Elements(2),Elements{4}); 
    ...
    for i = 1:length(elements)
        ...
        if isKey(atomicWeights, elementSymbol)
            atomicWeighta = atomicWeights(elementSymbol);
            molecularWeight = molecularWeight + (atomicWeighta * elementCount);
        else
